 ...
 * Yahoo Finance backend for MyTrade — no API key needed.
 *
 * Routes:
 *   GET /api/fmp?action=test
 *   GET /api/fmp?action=history&ticker=AAPL&period=1m|3m|1y
 *   GET /api/fmp?ticker=AAPL
 */
...
async function yahooFetch(ticker) {
  const { cookies, crumb } = await getYahooCookieAndCrumb();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'history') {
      ...
      const data = await yahooHistory(ticker, days);
      const chart = data?.chart?.result?.[0];
      if (!chart) return res.status(200).json([]);
    ...
    if (cache[ticker] && Date.now() - cache[ticker].fetchedAt < CACHE_TTL) {
      console.log(`[MyTrade] Cache hit — ${ticker}`);
      return res.status(200).json(cache[ticker]);
    }
    ...
    const sector   = r.summaryProfile?.sector          || '';
    const industry = r.summaryProfile?.industry        || '';
    ...
    console.log(`[MyTrade] OK — ${ticker} $${price} | EPS:${eps} | FCF:${fcf.toFixed(0)}M`);
