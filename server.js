  ...
  try {
    ...
    if (action === 'history') {
      ...
      const result = timestamps
        .map((t, i) => ({ date: new Date(t * 1000).toISOString().split('T')[0], close: parseFloat(closes[i]?.toFixed(2)) || 0 }))
        .filter(d => d.close > 0);
    ...
    console.log(`[MyTrade] OK — ${ticker} $${price} | EPS:${eps} | FCF:${fcf.toFixed(0)}M | Shares:${shares.toFixed(0)}M`);
